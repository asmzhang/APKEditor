...
# APKEditor
### 强大的安卓 APK 资源编辑器
该工具利用 [ARSCLib](https://github.com/REAndroid/ARSCLib) 来编辑任何 APK 资源，并具备六项主要功能
...
Commands:
  d | decode      将android资源二进制解码为可读的json/xml/raw。
  b | build       从json/xml/raw构建android二进制文件。
  m | merge       从目录或压缩后的apk文件中合并分离出来的apk文件.比如XAPK， APKM, APKS…
  x | refactor    重构混淆的资源名称
  p | protect     保护/混淆apk资源文件。使用独特的模糊技术。
  info            打印apk信息。
Other options:
  -h | -help      显示帮助并退出
  -v | -version   显示版本信息并退出
...
#### 1- Decompile
* 反编译apk资源为人类可读的json字符串
* 反编译apk资源到XML源代码（仅适用于未混淆的apk）。使用'''-t xml'''
...
Usage:
  d [Options, flags]
Options:
  -framework            framework 文件的路径（可以是多个）。
  -framework-version    首选 framework 版本号
  -i                    输入路径。
  -load-dex             一次加载dex文件的数量。
                        如果apk dex文件的数量大于这个值，
                        然后解码器每次加载一个dex。
                        *仅当-dex-lib设置为internal时生效。
                        *默认值= 3
                          *See<Notes> below.
  -o                    输出路径。可选，如果没有提供，则创建一个新文件将在输入的同一个目录上生成
  -remove-annotation    [Baksmali] dex中要删除的注释的类型名称
                        例如：-remove-annotation Lkotlin/Metadata；
                        *可以是多个。
  -res-dir              设置资源文件的根目录名称。例如，对于混淆，将文件从` res/* `移动到` r/* `或反之。
  -sig                  签名目录路径。
  -t                    解码类型:
                        [xml, json, raw, sig]
  -dex-lib              Dex库使用：
                        1)内部：使用内部库，支持dex
                        版本到042。
                        2) jf：使用JesusFreke/smali库，支持dex
                        版本035及以下。
                        *默认值= internal
                          *See <Notes> below.
                        [internal, jf]
  -comment-level        [Baksmali]设置评论级别
                        1)off：不打印注释。
                        2) basic：打印基本id和资源id注释。
                        3)detail：包括<basic>，打印类和方法关系的评论。
                        4) detail2：包含<detail>，打印十六进制编码字符串的字面量作为注释。
                        5) full：打印所有注释。
                        **默认值= detail
                        [off, basic, detail, detail2, full]
Flags:
  -dex                  复制原始dex文件/跳过smali。
  -dex-markers          转储dex标记（仅在smali模式下适用）。
  -dex-profile          将assets/dexopt下的dex profile二进制文件解码可读的json文件。
  -f                    强制删除输出路径
  -h | -help | --help   显示帮助并退出。
  -keep-res-path        保留res/*文件的原始路径：
                        *仅在解码xml时适用
                        *所有res/*文件将放置在目录<res-files>中
                        *相对路径将链接到values/*xml
  -no-cache             不创建dex .cache文件。
  -no-dex-debug         删除smali/dex中的所有调试信息。
